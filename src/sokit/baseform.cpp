...
void BaseForm::bindBuffer(qint32 id, QLineEdit *e, QToolButton *s, QComboBox *d) {
  ...
  bindClick(s, QKeySequence(Qt::CTRL, Qt::Key_0 + id));
  bindFocus(e, QKeySequence(Qt::ALT, Qt::Key_0 + id));                    // Qt::Key_0 + id | Qt::ALT);
  bindFocus(d, QKeySequence(Qt::CTRL, Qt::SHIFT, Qt::Key_0 + id));        // Qt::Key_0 + id | Qt::CTRL | Qt::SHIFT);
...
void BaseForm::bindFocus(QWidget *w, QKeySequence k) {
...
// }
void BaseForm::bindClick(QAbstractButton *b, QKeySequence k) {
  auto s = new QShortcut(k, this);
  connect(s, SIGNAL(activated()), b, SLOT(click()));
}

void BaseForm::bindSelect(QComboBox *b, qint32 i, QKeySequence k) {
  auto s = new QShortcut(QKeySequence(k), this);
