...
bool ServerForm::initHotkeys() {
  ...
  bindClick(m_ui.btnTcp, QKeySequence(Qt::CTRL | Qt::Key_T));
  bindClick(m_ui.btnUdp, QKeySequence(Qt::CTRL | Qt::Key_U));
...
void ServerForm::send(const QString &data, const QString &) {
  ...
  while (!list.isEmpty()) {
    ...
    ServerSkt *server = key.contains(tcpname) ? static_cast<ServerSkt *>(&m_tcp) : static_cast<ServerSkt *>(&m_udp);
