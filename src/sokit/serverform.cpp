...
ServerForm::ServerForm(QWidget *parent, Qt::WindowFlags flags) : BaseForm(parent, flags) { m_ui.setupUi(this); }
...
bool ServerForm::initForm() {
  ...
  connect(m_ui.btnTcp, &QToolButton::clicked, this, &ServerForm::trigger);
  connect(m_ui.btnUdp, &QToolButton::clicked, this, &ServerForm::trigger);
  ...
  connect(&m_tcp, &ServerSktTcp::connOpen, this, QOverload<const QString &>::of(&ServerForm::listerAdd));
  connect(&m_tcp, &ServerSktTcp::connClose, this, QOverload<const QString &>::of(&ServerForm::listerRemove));
  // WARNING: 此处连接两个信号
  connect(&m_tcp, &ServerSktTcp::message, this, QOverload<const QString &>::of(&ServerForm::output));
  connect(&m_tcp, &ServerSktTcp::dumpbin, this,
          QOverload<const QString &, const char *, quint32>::of(&ServerForm::output));
  connect(&m_tcp, &ServerSktTcp::countRecv, this, &ServerForm::countRecv);
  connect(&m_tcp, &ServerSktTcp::countSend, this, &ServerForm::countSend);

  connect(&m_udp, &ServerSktUdp::connOpen, this, QOverload<const QString &>::of(&ServerForm::listerAdd));
  connect(&m_udp, &ServerSktUdp::connClose, this, QOverload<const QString &>::of(&ServerForm::listerRemove));
  // WARNING: 此处连接两个信号
  connect(&m_udp, &ServerSktUdp::message, this, QOverload<const QString &>::of(&ServerForm::output));
  connect(&m_udp, &ServerSktUdp::dumpbin, this,
          QOverload<const QString &, const char *, quint32>::of(&ServerForm::output));
  connect(&m_udp, &ServerSktUdp::countRecv, this, &ServerForm::countRecv);
  connect(&m_udp, &ServerSktUdp::countSend, this, &ServerForm::countSend);
...
void ServerForm::send(const QString &data, const QString &) {
  ...
  while (!list.isEmpty()) {
    ...
    ServerSkt *server = key.contains(tcpname) ? (ServerSkt *)&m_tcp : (ServerSkt *)&m_udp;
