...
ServerSkt::ServerSkt(QObject *parent) : QObject(parent), m_port(0) { m_started = false; }
...
void ServerSkt::setError(const QString &err) { m_error = err; }

void ServerSkt::recordRecv(qint32 bytes) { emit countRecv(bytes); }

void ServerSkt::recordSend(qint32 bytes) { emit countSend(bytes); }

void ServerSkt::getKeys(QStringList &res) { res = m_conns.keys(); }
...
void *ServerSkt::getCookie(const QString &k) { return m_conns.value(k); }
...
void ServerSkt::show(const QString &msg) { emit message(msg); }

ServerSktTcp::ServerSktTcp(QObject *parent) : ServerSkt(parent) {}
...
void ServerSktTcp::newConnection() {
  ...
  while (client) {
    ...
    if (!conn) {
      ...
      connect(client, QOverload<QObject *>::of(&QTcpSocket::destroyed), this,
              QOverload<QObject *>::of(&ServerSktTcp::close));
...
void ServerSktTcp::send(void *cookie, const QByteArray &bin) {
  ...
  if (writeLen != srcLen) {
    show(QString("failed to send data to %1:%2 [%3]").arg(addr().toString()).arg(port()).arg(writeLen));
    return;
  }
...
ServerSktUdp::ServerSktUdp(QObject *parent) : ServerSkt(parent) {}
...
void ServerSktUdp::send(void *cookie, const QByteArray &bin) {
  ...
  while (ioLen > 0) {
    ...
    ioLen =
        (writeLen >= srcLen) ? 0 : m_server.writeDatagram(src + writeLen, srcLen - writeLen, conn->addr, conn->port);
  }

  if (writeLen != srcLen) {
    show(QString("failed to send data to %1:%2 [%3]").arg(conn->addr.toString()).arg(conn->port).arg(writeLen));
    return;
  }
