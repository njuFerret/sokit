...
Sokit::Sokit(int &argc, char **argv) : QApplication(argc, argv) {}

Sokit::~Sokit() {}

void Sokit::show() { m_wnd.show(); }

void Sokit::close() { m_wnd.close(); }
...
bool Sokit::initTranslator() {
  ...
  QStringList paths;
  paths << "."
        << "../share/" SET_APP_NAME << "../share/apps/" SET_APP_NAME << Setting::path();
...
bool Sokit::initUI() {
  ...
  return server->init() && transf->init() && client->init() && npd->init();
}
