...
TransferSkt::TransferSkt(QObject *parent) : QObject(parent), m_spt(0), m_dpt(0) {}
...
void TransferSkt::setError(const QString &err) { m_error = err; }

void TransferSkt::recordRecv(qint32 bytes) { emit countRecv(bytes); }

void TransferSkt::recordSend(qint32 bytes) { emit countSend(bytes); }

void TransferSkt::getKeys(QStringList &res) { res = m_conns.keys(); }
...
void *TransferSkt::getCookie(const QString &k) { return m_conns.value(k); }
...
void TransferSkt::show(const QString &msg) { emit message(msg); }

TransferSktTcp::TransferSktTcp(QObject *parent) : TransferSkt(parent) {}

TransferSktTcp::~TransferSktTcp() { m_server.disconnect(this); }
...
void TransferSktTcp::asynConnection() {
  ...
  show(QString("connection %1 to %2:%3 opened!").arg(conn->key, s->peerName()).arg(s->peerPort()));
}
...
void TransferSktTcp::send(void *cookie, bool s2d, const QByteArray &bin) {
  ...
  if (writeLen != srcLen) {
    show(QString("failed to send data to %1:%2 [%3]").arg(a.toString()).arg(p).arg(writeLen));
    return;
  }
...
TransferSktUdp::TransferSktUdp(QObject *parent) : TransferSkt(parent) {}

TransferSktUdp::~TransferSktUdp() { m_server.disconnect(this); }
      ...
      if (!conn) {
        ...
        if (conn) {
          ...
          if (!dst) {
            ...
            connect(dst, &QTcpSocket::readyRead, this, &TransferSktUdp::newData);
            // connect(dst, SIGNAL(destroyed(QObject *)), this, SLOT(close(QObject *)));
            connect(dst, &QTcpSocket::destroyed, this, qOverload<QObject *>(&TransferSktUdp::close));
            // WARNING connect(dst, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(error()));
            connect(dst, &QTcpSocket::errorOccurred, this, &TransferSktUdp::error);
