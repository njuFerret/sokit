...
ClientSkt::ClientSkt(QObject *parent) : QObject(parent), m_port(0) {}
...
void ClientSkt::setError(const QString &err) { m_error = err; }

void ClientSkt::recordRecv(qint32 bytes) { emit countRecv(bytes); }

void ClientSkt::recordSend(qint32 bytes) { emit countSend(bytes); }
...
void ClientSkt::show(const QString &msg) { emit message(msg); }

ClientSktTcp::ClientSktTcp(QObject *parent) : ClientSkt(parent) {}
...
void ClientSktTcp::asynConn() { show(QString("TCP connection to %1:%2 opened!").arg(addr().toString()).arg(port())); }

void ClientSktTcp::closed() { show(QString("TCP connection closed!")); }
...
ClientSktUdp::ClientSktUdp(QObject *parent) : ClientSkt(parent) {}

ClientSktUdp::~ClientSktUdp() {}

void ClientSktUdp::asynConn() { show(QString("UDP channel to %1:%2 opened!").arg(addr().toString()).arg(port())); }

void ClientSktUdp::closed() { show(QString("UDP channel closed!")); }
