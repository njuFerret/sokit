...
Logger::Logger(QObject *parent) : QObject(parent), m_chkWrite(nullptr), m_treeOut(nullptr), m_textOut(nullptr) {}
...
void Logger::init(QTreeWidget *o, QCheckBox *w, QPlainTextEdit *d) {
  ...
  if (m_treeOut && m_textOut && m_chkWrite) {
    ...
    connect(clear, &QAction::triggered, this, &Logger::clearLog);

    auto all = new QAction(tr("Select All"), this);
    all->setShortcuts(QKeySequence::SelectAll);
    connect(all, &QAction::triggered, m_textOut, &QPlainTextEdit::selectAll);
    ...
    connect(m_treeOut, &QTreeWidget::customContextMenuRequested, this, &Logger::ctxmenu);
    ...
    connect(m_textOut, &QTreeWidget::customContextMenuRequested, this, &Logger::ctxmenu);
  }
}
...
void Logger::ctxmenu(const QPoint &pos) {
  if (static_cast<QTreeWidget *>(sender()) == m_treeOut)
...
const QString Logger::getLogFileName() {
  ...
  return (i == 2) ? QString() : m_dir + QDir::separator() + QDate::currentDate().toString("yyyyMMdd.log");
}
...
void Logger::output(const QString &info) { output("MSG", info); }

void Logger::output(const char *buf, uint len) { output("DAT", buf, len); }
...
void Logger::output(const QString &title, const QString &info) {
  ...
  QScopedPointer<QTreeWidgetItem> it(new QTreeWidgetItem);
  if (it.isNull())
    return;
