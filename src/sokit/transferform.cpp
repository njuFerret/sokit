...
TransferForm::TransferForm(QWidget *parent, Qt::WindowFlags flags) : BaseForm(parent, flags), m_server(nullptr) {
  m_ui.setupUi(this);
}
...
bool TransferForm::initHotkeys() {
  ...
  bindClick(m_ui.btnTrigger, QKeySequence(Qt::CTRL, Qt::Key_S));
  bindSelect(m_ui.cmbType, 0, QKeySequence(Qt::CTRL, Qt::Key_T));
  bindSelect(m_ui.cmbType, 1, QKeySequence(Qt::CTRL, Qt::Key_U));
...
void TransferForm::trigger(bool start) {
  if (lock(1000)) {
    ...
    if (start) {
      start =
          TK::popIPAddr(m_ui.cmbSrcAddr, m_ui.cmbSrcPort, sa) && TK::popIPAddr(m_ui.cmbDstAddr, m_ui.cmbDstPort, da);
    }
...
void TransferForm::stop() { trigger(false); }
