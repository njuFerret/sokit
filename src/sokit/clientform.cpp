...
ClientForm::ClientForm(QWidget *p, Qt::WindowFlags f) : BaseForm(p, f), m_client(nullptr) { m_ui.setupUi(this); }
...
bool ClientForm::plug(bool istcp) {
  ...
  if (res) {
    ...
    if (!m_client) {
      ...
      connect(m_client, &ClientSkt::message, this, qOverload<const QString &>(&ClientForm::output));
      connect(m_client, &ClientSkt::dumpbin, this,
              qOverload<const QString &, const char *, quint32>(&ClientForm::output));
